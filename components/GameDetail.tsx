...
import { bestFromScores, combineScores, getSeedRows, useScores } from "@/lib/scores";

export function GameDetail({ game }: { game: Game }) {
  const router = useRouter();
  const seed = game.id.length * 17 + 3;
  const realScores = useScores();
  const best = useMemo(() => bestFromScores(game, realScores), [game, realScores]);
  const scores = useMemo(
    () => combineScores(getSeedRows(game.id, seed, 10), realScores, game.id, 10),
    [game.id, seed, realScores]
  );
